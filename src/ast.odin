...
// Create backreference node
make_backref :: proc(arena: ^Arena, num: int, name: string) -> ^Regexp {
	node := (^Regexp)(arena_alloc(arena, size_of(Regexp)))
	node^ = Regexp{
		op = .OpBackref,
		flags = {},
		data = arena_alloc(arena, size_of(Backref_Data)),
	}
	
	backref_data := (^Backref_Data)(node.data)
	backref_data^ = Backref_Data{num = num, name = name}
	
	return node
}

// Create lookahead assertion node
make_lookahead :: proc(arena: ^Arena, positive: bool, sub: ^Regexp) -> ^Regexp {
	node := (^Regexp)(arena_alloc(arena, size_of(Regexp)))
	node^ = Regexp{
		op = .OpLookahead,
		flags = {},
		data = arena_alloc(arena, size_of(Lookahead_Data)),
	}
	
	lookahead_data := (^Lookahead_Data)(node.data)
	lookahead_data^ = Lookahead_Data{positive = positive, sub = sub}
...
can_match_empty :: proc(node: ^Regexp) -> bool {
	...
	case .OpConcat:
		...
		return false
	case .OpBackref:
		// Backreferences match what was captured, so they can be empty if the capture was empty
		return true
	case .OpLookahead:
		// Lookahead assertions are zero-width
		return true
	}
	return false
}
...
count_captures :: proc(node: ^Regexp) -> int {
	...
	case .OpLookahead:
		lookahead_data := (^Lookahead_Data)(node.data)
		return count_captures(lookahead_data.sub)
	}
	return 0
}
...
simplify :: proc(arena: ^Arena, node: ^Regexp) -> ^Regexp {
	...
	case .OpLookahead:
		lookahead_data := (^Lookahead_Data)(node.data)
		simplified_sub := simplify(arena, lookahead_data.sub)
		if simplified_sub == nil {
			return nil
		}
		return make_lookahead(arena, lookahead_data.positive, simplified_sub)
	}
