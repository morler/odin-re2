...
clone_node :: proc(node: ^Regexp, arena: ^Arena) -> ^Regexp {
	...
	case .OpBackref:
		backref_data := (^Backref_Data)(node.data)
		return make_backref(arena, backref_data.num, backref_data.name)
	
	case .OpLookahead:
		lookahead_data := (^Lookahead_Data)(node.data)
		sub_cloned := clone_node(lookahead_data.sub, arena)
		return make_lookahead(arena, lookahead_data.positive, sub_cloned)
...
validate_ast :: proc(node: ^Regexp) -> ErrorCode {
	...
	case .OpBackref:
		...
		// Validate that either num or name is set
		if backref_data.num == 0 && len(backref_data.name) == 0 {
			return .InternalError
		}
	
	case .OpLookahead:
		lookahead_data := (^Lookahead_Data)(node.data)
		if lookahead_data == nil || lookahead_data.sub == nil {
			return .InternalError
		}
		return validate_ast(lookahead_data.sub)
...
parse_group :: proc(p: ^Parser) -> ^Regexp {
	...
	if !at_end(p) && peek(p) == '?' {
		advance(p) // Consume '?'
		if at_end(p) {
			return nil // Incomplete group specification
		}
	...
	if is_lookahead {
		// Lookahead assertion
		return make_lookahead(p.arena, lookahead_positive, node)
...
// Parse backreference number
parse_backref_number :: proc(p: ^Parser) -> (int, bool) {
	if at_end(p) {
		return 0, false
	}
	...
	for !at_end(p) && digits < 3 && is_digit(byte(peek(p))) {
		digit := int(peek(p) - '0')
		num = num * 10 + digit
		advance(p)
		digits += 1
...
parse_escape :: proc(p: ^Parser) -> ^Regexp {
	...
	case 'S': return make_char_class(p.arena, []Char_Range{{' ', ' '}, {'\t', '\t'}, {'\n', '\n'}, {'\r', '\r'}, {'\f', '\f'}, {'\v', '\v'}}, true)
	case 'b': return make_anchor(p.arena, .OpWordBoundary)
	case 'B': return make_anchor(p.arena, .OpNoWordBoundary)
	case 'g':
		// Parse \g{name} or \g<number> named/numeric backreference
		if !at_end(p) && peek(p) == '{' {
			advance(p) // Consume '{'
			if at_end(p) {
				return nil // Unclosed \g{
			}
			
			// Check if it's a number or name
			if is_digit(byte(peek(p))) {
				num, ok := parse_backref_number(p)
				if !ok || at_end(p) || peek(p) != '}' {
					return nil // Invalid \g{n} syntax
				}
				advance(p) // Consume '}'
				return make_backref(p.arena, num, "")
			} else {
				// Parse named backreference \g{name}
				name_start := p.pos
				for !at_end(p) && peek(p) != '}' && peek(p) != '}' {
					advance(p)
				}
				if at_end(p) || peek(p) != '}' {
					return nil // Unclosed \g{name}
				}
				name := p.pattern[name_start:p.pos]
				advance(p) // Consume '}'
				return make_backref(p.arena, 0, name)
			}
		} else {
			// Invalid \g syntax
			return nil
	...
	case:
		// Check for numeric backreference \1, \2, etc.
		if is_digit(byte(ch)) {
			// Put the digit back to parse the full number
			p.pos -= 1
			num, ok := parse_backref_number(p)
			if ok && num > 0 {
				return make_backref(p.arena, num, "")
			}
			// If not a valid backreference, treat as literal
...
parse_quantified_term :: proc(p: ^Parser) -> ^Regexp {
	...
	case '*':
		...
		return make_repeat(p.arena, .OpStar, base, 0, -1, non_greedy)
	case '+':
		advance(p)
		// Check for lazy quantifier +?
		non_greedy := false
		...
		return make_repeat(p.arena, .OpPlus, base, 1, -1, non_greedy)
	case '?':
		advance(p)
		// Check for lazy quantifier ??
		non_greedy := false
		...
		return make_repeat(p.arena, .OpQuest, base, 0, 1, non_greedy)
	case '{':
...
parse_repeat :: proc(p: ^Parser, base: ^Regexp) -> ^Regexp {
	...
	return make_repeat(p.arena, .OpRepeat, base, min, max, non_greedy)
}
